<?php
	...
	if (isset($_POST["Matchid"]) and isset($_POST["Date"]) and isset($_POST["Time"]) and isset($_POST["duration"]) and isset($_POST["location"]) and isset($_POST["capacity"]))
	{
		$id = filter_var($_POST["Matchid"],FILTER_SANITIZE_STRING);
		...
		try
		{
			...
			if ($result['Spaces'] > $capacity ) 
			{
				echo "<script type='text/javascript'> alert('The spaces of this game is more than the capacity!') </script>";
				echo "<meta http-equiv='Refresh' content='0;URL=adminmatch.php'>";
				exit(0);
			}

			
			$stmt = $pdo->prepare("UPDATE Matches SET `Matchdate` = :dat, `Starttime` = :tim, `Duration` = :duration, `Location` = :location, `Capacity` = :capacity, `Information` = :information where `MatchID` = :id");
			$stmt->bindParam(':id', $id);
